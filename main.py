...
#        print(np.shape(taus_tot_b), np.shape(tau_data), flush=True)
        ...
        for ind_data, (flux_line,tau_line, spec_line) in enumerate(
            ...
            #News
                 data_to_get = np.log10(np.abs(1e19*(spec_line[:,bin_min:len(bins)]).T))
            ...
            if like_on_flux is not False:
                ...
                #        raise IndexError
                likelihood_spec[:ind_data] += np.log(spec_kde.evaluate(np.log10(np.abs(1e19*(like_on_flux[ind_data][bin_min:len(bins)])).reshape(len(bins)-bin_min,1))))
